...
function validate_element_field(key, element_id, field_name, value) {
    ...
    else if(field_model.type === "ipnetwork") {
        let network = IP.network.create(value);
        if(network === null)
            return `Field '${field_name}' value invalid.`;

        // Check if object needs to be unique
        if(field_model.unique) {
            let network_string = IP.network.toLongString(network);
            for(let other_element_id in db.keys[key]) {
                if((element_id !== other_element_id) && (db.keys[key][other_element_id].fields[field_name] === network_string))
                    return `Field '${field_name}' duplicated.`;
            }
        }
    }
...
function view_get_key_list(req, res) {
    ...
    if(req.query.length === 0)
        return res.status(200).json(db.keys[key]);

    // Check query parameters
    for(let param_name in req.query) {
        if((param_name !== "id") && (!(param_name in model_definition.keys[key].fields)))
            return res.status(403).json({error: "Query error."});
    }

    // Build the result
    let result = {};
    for(let id in db.keys[key]) {
        let entry = db.keys[key][id];
        let match = true;
        for(let param_name in req.query) {
            if(req.query[param_name] != entry.fields[param_name]) {
                match = false;
                break;
            }
        }
...
function view_new_key_element(req, res) {
    ...
    for(let field_name in model.fields) {
        ...
        if(["ip", "ipnetwork", "string", "name", "text", "email", "number", "mac_address"].indexOf(field_model.type) !== -1) {
            // Get value of field
            ...
            if(field_model.type === "ipnetwork") {
                // For networks, we store the long representation only (in case ipv6 has :: )
                fields[field_name] = IP.network.toLongString(IP.network.create(fields[field_name]));
            }
        }
        else if(field_model.type === "reference") {
            if(field_name in req.body) {
    ...
    // Apply external post processing
    if(key in external_post_process) {
        for(let f of external_post_process[key]) {
            f("new", key, new_id, db.keys[key][new_id]);
        }
    }
...
function view_update_key_element(req, res) {
    ...
    if(typeof req.body.fields === "object") {
        ...
        let element = db.keys[key][element_id];
        let old_element = tools.deep_copy_json(element);
        ...
        for(let field_name in req.body.fields) {
            ...
            if(["string", "name", "text", "email", "number", "mac_address"].indexOf(field_model.type) !== -1) {
                ...
                // For networks, we store the long representation only (in case ipv6 has :: )
                element.fields[field_name] = IP.network.toLongString(IP.network.create(req.body.fields[field_name]));
            }
            else if(field_model.type === "reference") {
        ...
        if(key in external_post_process) {
            for(let f of external_post_process[key]) {
                f("update", key, element_id, element, old_element);
            }
        }
...
function view_delete_key_element(req, res) {
    ...
    let deleted_element = db.keys[key][element_id];
    delete db.keys[key][element_id];

    // Apply external post processing
    if(key in external_post_process) {
        for(let f of external_post_process[key]) {
            f("delete", key, element_id, deleted_element);
        }
    }
...
function add_external_postprocess(key, f) {
    if(!(key in external_post_process))
        external_post_process[key] = [f];
    else
        external_post_process[key].push(f);
}
...
module.exports = {
    ...
    add_external_postprocess: add_external_postprocess,
    db: () => {return db},
};